...
async function createUser(name) {
	try {
		if (!await findOneByName(name)) {
			const result = await client.db("restricter").collection("users").insertOne({"name": name, "restricted_domains": {}})
...
async function findOneByName(name) {
	try {
		return await client.db("restricter").collection("users").findOne({"name": name});
	} catch(err) {
		console.log(err.message)
	}
}
...
app.post('/question/:name', async (req, res) => {
